...
plus reflection.}~\sidecite{DBLP:conf/cade/AllenCEKL00} and more recently in
\Andromeda~\sidecite{andromeda}.
%
...
pioneering work of \sidecite{hofmann1995conservativity,HofmannPhD},
%
...
\sidecite{oury2005extensionality}. However, his presentation does not give
...
in~\sidecite{boulier17:next-syntac-model-type-theor} and has been entirely
programmed and
formalised in
\Coq~\sidecite{coq}. For this, we rely on
\TemplateCoq\sidenote{\url{https://metacoq.github.io/metacoq/}}~\sidecite{DBLP:conf/itp/AnandBCST18},
...
whenever they are pointwise equal\sidenote{In Homotopy Type
  Theory (HoTT)~\sidecite{hottbook}, FunExt is
...
Streicher using the groupoid model~\sidecite{groupoid-interp}, which has
recently been extended in the setting of univalent type theory using
the simplicial or cubical models~\sidecite{kapulkin2012simplicial,coquand:cubical}.
%
Similarly, FunExt is independent from ITT,
it is folklore but has recently been formalised by Boulier \emph{et al.} using a
simple syntactical translation~\sidecite{boulier17:next-syntac-model-type-theor}.
...
equality} introduced by \sidecite{mcbride2000dependently}.
%
However, to avoid adding additional axioms to ITT as done by
\sidecite{oury2005extensionality}, we prefer to encode this
...
\sidecite{bernardy2012proofs} and recently rephrased in the setting of
\TemplateCoq~\sidecite{DBLP:conf/itp/AnandBCST18}. Namely, a context is not
...
\sidecite{streicher1993investigations} for a similar example).
%
...
(and that of Martin \sidecite{hofmann1995conservativity} who has a similar
presentation) and the work of \sidecite{oury2005extensionality}.
...
in the setting of \TemplateCoq~\sidecite{DBLP:conf/itp/AnandBCST18} in order to have
