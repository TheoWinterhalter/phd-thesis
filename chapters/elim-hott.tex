...
\marginnote[2cm]{
  Refer to \nrefch{flavours} for more on \acrshortpl{2TT}.
}
Homotopy and reflection seem contradictory at first since \acrshort{UIP}
often comes with reflection\sidenote{\acrshort{UIP} can be proved from
reflection and the \(\mathsf{J}\) eliminator of equality.} and is negated by
univalence, but as I have exposed earlier, using \acrlongpl{2TT} we can make two
...
\marginnote[-0.2cm]{
  ...
  well. As one can imagine it does not change the proof much.
}
...
\marginnote[-1cm]{
  ...
  (because translations are type-preserving).
}
