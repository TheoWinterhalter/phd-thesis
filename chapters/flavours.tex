...
\Coq is originally based on the Calculus of Constructions (\CoC) though it is
now rather called the \acrfull{PCUIC}.
\todo{What to say?}

\section{Equality in type theories}

\todo{Re-present the inductive eq then HoTT, talk about alternatives like OTT
and cubical, then present two-level type theories and HTS.}