...
In fact, pattern-matching is the only way to express elimination in \Coq.

This rule might seem a bit useless, but essentially it means that to prove
anything involving a dependency on \(\unit\), like \(P[x \sto u]\), it suffices
to prove it assuming it is \(\tunit\): \(P[x \sto \tunit]\).
...
I already presented lists in \ocaml, and their \Coq counterpart is not much
different.
\begin{mathpar}
  \infer
    {\Ga \vdash A}
...
There is also a dependent varaint of \acrshort{funext}.
\[
  \begin{array}{lcl}
    \mathsf{funextD} &:&
...
recover the property that heterogenous equalities where the two types are in
fact the same like \(\Heq{A}{u}{A}{v}\), imply homogenous equality \(u =_A v\).
This has to be added as an extra axiom, equivalent to \acrshort{UIP}.

For this reason, I prefer an encoding of heterogenous equality with
\(\Sigma\)-types:
