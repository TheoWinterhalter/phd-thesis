...
\begin{mathpar}
  \infer
    ...
    {\Ga \vdash \left(\pmatch{t}{x.P}{}\right) : P[x \sto t]}
  %
\end{mathpar}
...
% Inductive types are morally types of trees. For now I only presented types
...
\begin{itemize}
  ...
  \marginnote[0.6cm]{
    \(\mathbf{C}\) stands for a constructor like \(\zero\) or \(\natsucc\).
  }
  \[
    \begin{array}{lc}
      (\fixp_n (f : \Pi \D.T).\ t)\
...
\subsection{Parametrised inductive types}

Parametrised inductive types, as the name suggests, can take parameters.
...
\marginnote[0.1cm]{
  The `positive'/`negative' terminology is not standard but I believe it to
  be fairly usual in my field of expertise.
}
...
positive version: \mintinline{coq}{pair}.
...
be deactivated with an option~\sidecite{cockx2016eliminating} which restricts
pattern-matching.
