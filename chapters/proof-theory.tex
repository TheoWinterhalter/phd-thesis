...
a finite amount of time, it's good, but if it takes ages, we will not have any
certainty. In \sidecite{de1991plea}, de Bruijn suggests that a proof should be
self-evident. You should not have to think for hours before seeing that is indeed
...
\marginnote[0.2cm]{
  ...
  propositions and just introduction rules it is instead called the
  \emph{sequent calculus}.
}
...
know about it:
\begin{itemize}
  \item Is it \emph{consistent}? An inconsistent system is one in which every
