...
\marginnote[0.2cm]{
  This setting with only one proposition on the right is called
  \emph{natural deduction} and was introduced by Gentzen;
  this is not the only possible presentation of judgements and inference rules,
  for instance Gentzen also introduced the \emph{sequent calculus} where there
  can be several propositions on the right-hand side.
}
...
\todo{Rewrite the rest in natural deduction}
...
might seem a bit peculiar. If we forget about \(\D\) and \(\Xi\), it says that
if \(\Ga\) entails \(A\) which itself entails \(\Theta\) then \(\Ga\) entails
...
\LK is classical because we can derive the \acrfull{LEM} from it.
\begin{mathpar}
  \infer*[Right=\((CR)\)]
    {
      \infer*[Right=\((\vee R_1)\)]
...
Intuitionistic logic does not abide by the \acrshort{LEM}, so its treatment of
...
We have less rules because some are no longer useful with only one
proposition on the right-hand side.
\begin{mathpar}
  \infer
    {\Ga, A, B \vdash C}
