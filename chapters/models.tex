...
theory in which the model lives... At least in principle. It is however very
rare\sidenote{This may work with very simple theories that cannot express
arithmetic, as explained in \nrefsec{limit-godel} of \refch{proof-theory}} to
\emph{know} that a theory is consistent, instead we should see it as a theory we
\emph{trust}, typically a theory that is widely accepted as being consistent by
...
\reminder[-1.8cm]{Classical logic}{
  ...
  which consists of a proof of \(A \vee \neg A\).
}
...
subject reduction.
%, but such models are usually built for that purpose.
...
This is exposed
in~\sidecite[-0.9cm]{miquel2001calcul}---following~\sidecite[-0.3cm]{aczel1998relating,xi2000imperative}---in
...
\marginnote[0.3cm]{
  This property is called \emph{soundness}.
}
...
The proof of soundness is done by looking at each rule and showing they preserve
...
\marginnote[1.1cm]{%
  Here I use type-theoretic notations: \(\Ga : \Con\) means \(\Ga\) an object
  of the catergory \(\Con\), \(\sigma : \Ga \to \D\) means \(\sigma\) a morphism
  from object \(\Ga\) to object \(\D\), etc.
}%
\begin{enumerate}
...
The category \(\Set\) of sets is a \acrshort{CwF}~\sidecite{hofmann1997syntax}:
\marginnote[3.8cm]{
...
\begin{enumerate}
  ...
  \item given \(\sigma : \Ga \to \D\) and \(A : \CTy\ \D\), the type
  \(A[\sigma]\) is defined as the \(\Ga\)-indexed family
  \((A_{\sigma(\gamma)})_\gamma\);
  \item given \(\sigma : \Ga \to \D\) and \(t : \CTm\ \D\ A\), the term
  \(t[\sigma]\) is defined as the choice function \(t \circ \sigma\);
  \item the empty context is given as the singleton \(\{ \emptyset \}\);
  \item given \(\Ga : \Con\) and \(A : \CTy\ \Ga\), the extended context
  \(\Ga, A\) is defined as the disjoint union
  \(\amalg_{\gamma \in \Ga} A_\gamma\), the morphism \(\pp\) and term \(\pq\)
  are defined as the first and second projection respectively;
  \item given \(\sigma : \Ga \to \D\) and \(a : \CTm\ \Ga\ A[\sigma]\),
  the substitution \(\sigma, a\) is defined as the function
...
\begin{itemize}
  ...
  \item for \(A : \CTy\ \Ga\) and \(u : \CTm\ \Ga\ A\) and
  \(C : \CTm\ (\Ga, A, \CId_{A[\pp]}\ u[\pp]\ \pq)\) and
  \(f : \CTm\ \Ga\ C[\cid, u, \crefl_u]\)
  and \(v : \CTm\ \Ga\ A\) and \(e : \CTm\ \Ga\ (\CId_A\ u\ v)\) there exists
  \(\cJ_A\ u\ C\ f\ v\ e : \CTm\ \Ga\ C[\cid, v, e]\)
\end{itemize}
such that
\[
  \begin{array}{rcl}
    ...
    (\cJ_A\ u\ C\ f\ v\ e)[\sigma] &=&
    \cJ_{A[\sigma]}\ u[\sigma]\ C[(\sigma \circ \pp, \pq) \circ \pp, \pq]\ f[\sigma]\ v[\sigma]\ e[\sigma]
  \end{array}
\]
% Several eliminators are possible for equality but \(\cJ\) is the most standard
% one.
...
\begin{itemize}
  ...
  \item given \(b : \CTm\ \Ga\ \bool\) and \(C : \CTy\ (\Ga, \bool)\) and
...
\begin{itemize}
  \item \(\nat : \CTy\ \Ga\);
  \item \(\czero : \CTm\ \Ga\ \nat\);
  \item for \(n : \CTm\ \Ga\ \nat\), there exists \(\csucc\ n : \CTm\ \Ga\ \nat\);
  \item for \(n : \CTm\ \Ga\ \nat\) and \(C : \CTy\ (\Ga, \nat)\) and
...
All of these constructions (dependent products, sums, natural numbers,
...
\marginnote[0.1cm]{
  The existence of such a bijection and this definition of cardinal rely on
...
\begin{enumerate}
  ...
  \item given \(\Ga : \Con\) and \(A : \CTy\ \Ga\), the extended context
  \(\Ga, A\) is defined as the cardinal of the disjoint union
  \(\card{\amalg_{\gamma \in \Ga} A_\gamma}\), the substitution \(\pp\) is
  defined as \(\pi_1 \circ \beta^{-1}\) where \(\pi_1\) is the first projection
  of the disjoint union, the term \(\pq\) is defined as
  \(\pi_2 \circ \beta^{-1}\);
  \item given \(\sigma : \Ga \to \D\) and \(a : \CTm\ \Ga\ A[\sigma]\), the
  substitution \(\sigma, a\) is defined as the function
...
Finally, given \(A : \CTy\ \Ga\) and \(u : \CTm\ \Ga\ A\) and
\(C : \CTm\ (\Ga, A, \CId_{A[\pp]}\ u[\pp]\ \pq)\) and
\(f : \CTm\ \Ga\ C[\cid, u, \crefl_u]\)
and \(v : \CTm\ \Ga\ A\) and \(e : \CTm\ \Ga\ (\CId_A\ u\ v)\)
we can build \(\cJ\) by first remarking that for any \(\gamma \in \Ga\)
we have \(e(\gamma) \in (\CId_A\ u\ v)(\gamma)\) meaning that \(e(\gamma) = 0\)
and that \(u(\gamma) = v(\gamma)\). As such we define it as
\[
  (\cJ_A\ u\ C\ f\ v\ e)(\gamma) \coloneqq f(\gamma)
\]
...
\marginnote[0.6cm]{
  The equality for \(\cJ\) is proven by going through \(f[\sigma](\gamma)\).
}
\[
  \begin{array}{rcl}
...
The cardinal model is even stronger than that because it validates isomorphism
reflection.
...
This consists of a strong invariance principle stating that isomorphic types
...
\sidedef[-0.7cm]{Inaccessible cardinal}{
...
\sidedef[-0.7cm]{Aleph number}{
  The aleph numbers are cardinals starting from \(\aleph_0\) the cardinal of
  natural numbers, \(\aleph_1\) being the next larger cardinal and so on\dots
  \(\aleph_\alpha\) is actually defined for any ordinal \(\alpha\) but it is
  beyond the scope of this thesis.
}
\marginnote[3.3cm]{
  This definition of universe works because inaccessible cardinals are
  fixed-points of the \(\aleph\) function.
}
