...
The substitutions equalities are fairly straightforward.
The computations are proven below.

\marginnote[0.9cm]{
  Since \(\czero(\gamma) = 0\).
}
\marginnote[1.9cm]{
  Since \((\csucc\ n)(\gamma) = n(\gamma) + 1\).
}
\[
  \begin{array}{rcl}
    (\natrec_C\ \czero\ z\ s)(\gamma)
    &=& z(\gamma) \\
    \\
    (\natrec_C\ (\csucc\ n)\ z\ s)(\gamma)
    &=&
    \capp(\capp(s, n), \natrec_C\ n\ z\ s)(\gamma)
  \end{array}
\]
