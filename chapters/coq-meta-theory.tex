...
\begin{center}
  \begin{tikzpicture}[baseline=(u.base), node distance=1cm]
    ...
    \path (d1.center) edge[to*, tred] (d2.center) ;
    \path (d3.center) edge[to*, tred] (d2.center) ;
...
\begin{center}
  \begin{tikzpicture}[baseline=(u.base), node distance=1cm]
    ...
    \path (d3.center) edge[to*, tred] (d2.center) ;
    \path (d3.center) edge[to*, tred] (v) ;
    \path (u) edge[to*, tred, dashed] (a.center) ;
    \path (d2.center) edge[to*, tred, dashed] (a.center) ;
    \path (d2.center) edge[to*, tred, dashed] (b.center) ;
    \path (v) edge[to*, tred, dashed] (b.center) ;
    \path (a.center) edge[to*, tred, dashed] (c.center) ;
    \path (b.center) edge[to*, tred, dashed] (c.center) ;
  \end{tikzpicture}
\end{center}
