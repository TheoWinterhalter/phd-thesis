...
anyway.}
\todo{Missing symbols for projection computation too!}
...
\marginnote[0.8cm]{
  Notice that in the typed conversion, \(v\) and \(u'\) were both \(u\),
  and \(A'\) was \(A\). This will still be the case for well-typed terms, but
  the reduction rule should not have to decide conversion to apply.
}
\begin{mathpar}
