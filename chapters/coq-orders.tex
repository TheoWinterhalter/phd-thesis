...
I did mention bisimulations however and didn't even talk about it.
...
\begin{definition}[Bisimulation]
  A relation \(\sim\) on \(A\) is a bisimulation for \(\red\) when it is both
  a simulation for \(\red\) and for \(\red^{-1}\).
\end{definition}

A bisimulation doesn't quite fit the bill yet though. The simulation property
...
\todo{This can probably be relaxed actually.}
...
\todo{Bisimularity is ER, then say dependent case more complex so need for coe
but I still kept the naming of well-known property.}